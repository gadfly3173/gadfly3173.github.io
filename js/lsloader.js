...
(function () {
  ...
  lsloader.clean = function () {
    try {
      ...
      keys.forEach((key) => {
        const data = lsloader.getLS(key);
        if (window.oldVersion) {
          const remove = window.oldVersion.reduce((p, c) => p || data.indexOf(`/*${c}*/`) !== -1, false);
          if (remove) {
  ...
  lsloader.load = function (jsname, jspath, cssonload, isJs) {
    ...
    // 取出对应文件名下的code
    if (code) {
      const versionNumber = code.split(versionString)[0]; // 取出路径版本号 如果要加载的和ls里的不同,清理,重写
      ...
      if (isJs) {
        this.jsRunSequence.push({ name: jsname, code });
  ...
  lsloader.runjs = function (path, name, code) {
    ...
    if (!!name && !!code) {
      for (const k in this.jsRunSequence) {
  ...
  lsloader.tagLoad = function (path, name) {
    this.jsRunSequence.push({ name, code: '', path, status: 'failed' });
    this.runjs();
  };
  ...
  lsloader.runInlineScript = function (scriptId, codeId) {
    ...
    this.jsRunSequence.push({ name: scriptId, code });
    this.runjs();
  };
  ...
  lsloader.loadCombo = function (jslist) {
    let updateList = '';// 待更新combo模块列表
    const requestingModules = {};// 存储本次更新map
    for (const k in jslist) {
      const LS = this.getLS(jslist[k].name);
      if (LS) {
      ...
      if (version == jslist[k].path) {
        this.jsRunSequence.push({ name: jslist[k].name, code, path: jslist[k].path }); // 缓存有效 代码加入runSequence
      } else {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
          if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304) {
            ...
            for (const i in that.jsRunSequence) {
